...
| `anniversary_type` | varchar(40), nullable | 예약별 기념일 유형 |
| `anniversary_date` | varchar(10), nullable | 예약별 기념일 날짜 |
| `request_template_values` | json, nullable | 고객이 선택한 요청사항 템플릿 값 |
| `marketing_opt_in` | boolean | 예약 시점 마케팅 수신 동의 여부 |
...
```json
{
  ...
  "movedNoteCount": 4,
  "anonymizedCustomerIds": ["cus_456", "cus_789"],
...
```json
{
  "customerId": "cus_123",
  "anonymized": true,
  "anonymizedName": "익명 고객",
  "anonymizedPhoneMasked": "900-****-0123",
  "deletedNoteCount": 2,
  "warning": "개인정보 익명화는 되돌릴 수 없으며, 예약 운영에 필요한 최소 이력만 유지됩니다."
}
```
...
- 병합된 고객 레코드는 즉시 삭제하지 않고 이름, 연락처, 선택 정보, 내부 메모, 플래그를 익명화해 식별 정보를 제거한다.
- 병합 요청은 `confirmIrreversible: true`를 요구하고, API 응답에 되돌릴 수 없다는 경고 문구를 포함한다.
- 병합 취소는 MVP에서 제공하지 않는다. 잘못 병합한 경우 데이터베이스 백업 기반의 별도 수동 복구 절차로만 다룬다.
...
- 공개 예약에서 입력된 이메일, 알레르기, 기념일은 고객 프로필에도 갱신하되, 예약별 요청사항과 요청사항 템플릿은 예약 스냅샷으로 보존하고 고객 선호 메모를 자동 덮어쓰지 않는다.
- 마케팅 수신 동의는 예약 시점의 동의 스냅샷으로 저장하며, 자동 캠페인 발송이나 고객 세그먼트 기능과 연결하지 않는다.
- 예약, 결제, 환불, 분쟁 대응에 필요한 기록은 관련 법령과 운영 정책에 따라 보존할 수 있다.
- 선택 정보와 내부 메모는 고객 삭제 요청 또는 운영 필요 종료 시 삭제/익명화한다.
- 개인정보 익명화 요청은 `confirmIrreversible: true`를 요구하며, 고객명/전화번호/이메일/알레르기/기념일/선호/내부 메모/VIP/주의/차단 정보를 제거한다.
- 익명화 시 예약/결제/환불 이력은 고객 ID 기준으로 유지하되, 고객 식별 정보와 활성 고객 메모는 제거한다.
